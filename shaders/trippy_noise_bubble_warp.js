...
void main(){
    ...
    st=st+(st*noise(vec2(st.x+sin(iTime/2.),st.y+cos(iTime/2.))))* pow(1.- (d), 2.);
    ...
    pct=distance(st,vec2(sin(iTime),cos(iTime)))/2.*noise(vec2(st.x/100.+sin(iTime/2.),st.y/100.+cos(iTime/2.)));
    green=distance(st,vec2(sin(iTime-.5),cos(iTime+3.5)))/2.;
